...
export async function POST(request: Request) {
  try {
    ...
    const userId = cookieStore.get("xcom_user_id")?.value;
    ...
    if (post.authorUserId !== userId) {
      return Response.json(
        { error: "Forbidden - only post author can retry" },
        { status: 403 }
      );
    }

    if (post.xSyncStatus !== "failed" && post.xSyncStatus !== "pending") {
      return Response.json(
        { error: "Cannot retry post with status: " + post.xSyncStatus },
        { status: 400 }
      );
    }
